...
def parse_args():
    ...
    parser.add_argument(
        "--json-file",
        default="../../../data/VSAI/test/annotations.json",
        help="Path to the annotation JSON file",
    )
    parser.add_argument(
        "--work-dir",
        default="./results/wo_c_task2",
        help="Directory to save results (masks, visualizations, json)",
    )
    parser.add_argument(
        "--model",
        default="./models/Aerial-r1-wo-c",
...
def main():
    ...
    for idx, item in enumerate(dataset_items):
        ...
        prompt = f"<image>Please segment all the vehicles."
        ...
        print(f"  GT objects: {len(gt_objects)}, Pred masks: {len(pred_masks_bin)}, IoU: {iou:.4f}")
